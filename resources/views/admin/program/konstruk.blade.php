...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl font-bold text-gray-900 dark:text-white">Program Exbis</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">List Project Konstruksi Eksterna;</p>
    </div>
    <!-- <div class="flex gap-2">
        <button type="button" onclick="openImportModal()" class="h-10 px-4 inline-flex items-center justify-center rounded-lg border border-gray-300 dark:border-gray-700 text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800">Import CSV</button>
        <button type="button" onclick="openProjectModal('EXBIS')" class="h-10 px-4 inline-flex items-center justify-center rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold">+ LOP Exbis Baru</button>
    </div> -->
</div>

{{-- Search & Filter --}}
<div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 mb-6 shadow-sm">
    <form method="GET" action="{{ route('program.konstruk') }}" class="space-y-4">
        <div class="flex flex-col lg:flex-row gap-3">
            ...
            <a href="{{ route('program.konstruk') }}" class="h-11 px-5 inline-flex items-center justify-center rounded-lg border border-gray-300 text-sm font-bold hover:bg-gray-100">Reset</a>
        </div>
...
@include('admin.program.partials.table', ['projects' => $projects, 'programName' => 'EXBIS'])
