...
<div class="max-w-4xl mx-auto space-y-4 px-4 py-6">
    ...
    <div class="flex items-start justify-between gap-3 flex-wrap">
        <div>
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Preview Dokumen BAUT</h1>
            <p class="text-sm text-gray-500">{{ $lop->lop_name ?? '-' }} · PID: {{ $project->pid ?? '-' }}</p>
        </div>
        <a href="{{ route('admin.pt2.baut.index') }}" class="h-10 px-4 rounded-xl border border-gray-300 dark:border-gray-700 inline-flex items-center text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
            ← Kembali ke Daftar
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            @foreach([
                ...
                'district' => 'District', 'lokasi' => 'Lokasi', 'tempat_tanggal' => 'Tempat, Tanggal',
                'tanggal_uji_terima' => 'Tanggal Uji Terima', 'keputusan_uji_terima' => 'Keputusan',
                'nama_tii' => 'Nama TII', 'nama_akses' => 'Nama Akses',
            ] as $key => $label)
        ...
        <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
            <a id="baut-download-link" href="{{ route('admin.pt2.baut.download', $record->id_baut_generate) }}" class="flex-1 h-11 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white inline-flex items-center justify-center text-sm font-black">
                📥 Download .docx
            </a>
            <a href="{{ route('admin.pt2.baut.editor', $record->pt2_lop_id) }}" class="flex-1 h-11 rounded-xl border border-slate-300 inline-flex items-center justify-center text-sm font-black text-slate-600 hover:bg-slate-50">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
        <div class="flex items-center justify-between mb-3">
            ...
            <span id="baut-preview-status" class="text-xs font-bold text-gray-400">Memuat preview...</span>
        </div>
        <div id="baut-preview-container" class="rounded-xl border border-gray-200 dark:border-gray-800 overflow-auto max-h-[75vh] bg-gray-100 dark:bg-gray-950 p-4"></div>
...
<script>
    // Tombol "Generate & Download" di editor otomatis memicu download begitu
    // dokumen selesai digenerate dan halaman ini terbuka.
    document.getElementById('baut-download-link')?.click();
</script>
@endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const statusEl = document.getElementById('baut-preview-status');
    const container = document.getElementById('baut-preview-container');
    ...
    fetch(url)
        ...
        .catch(function (err) {
            console.error('BAUT docx preview error:', err);
