...
<div class="max-w-6xl mx-auto space-y-4 px-4 py-6">
    ...
    <div class="flex items-start justify-between gap-3 flex-wrap">
        <div>
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Hasil Generate LACT</h1>
            <p class="text-sm text-gray-500">Riwayat dokumen Laporan Commissioning Test yang sudah/sedang dibuat per LOP.</p>
        </div>
    </div>
    ...
    <form method="GET" class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 shadow-sm flex flex-wrap items-end gap-3">
        ...
        <a href="{{ route('admin.pt2.lact.index') }}" class="h-11 px-5 rounded-xl border border-slate-300 inline-flex items-center text-sm font-black text-slate-600">Reset</a>
    </form>
                        ...
                        <tr class="align-top">
                            ...
                            <td class="p-3">
                                <div class="flex flex-wrap justify-end gap-2">
                                    @if($isFinal)
                                        <a href="{{ route('admin.pt2.lact.show', $record->id_lact_generate) }}" class="h-9 px-3 rounded-lg border border-slate-300 inline-flex items-center justify-center text-xs font-black text-slate-600 hover:bg-slate-50 whitespace-nowrap">
                                            Preview
                                        </a>
                                        <a href="{{ route('admin.pt2.lact.download', $record->id_lact_generate) }}" class="h-9 px-3 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white inline-flex items-center justify-center text-xs font-black whitespace-nowrap">
                                            Download
                                        </a>
                                    @else
                                        <a href="{{ route('admin.pt2.lact.editor', $record->pt2_lop_id) }}" class="h-9 px-3 rounded-lg bg-amber-500 hover:bg-amber-600 text-white inline-flex items-center justify-center text-xs font-black whitespace-nowrap">
                                            Lanjutkan Edit
                                        </a>
                                    @endif
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-10 text-center">
                                <p class="text-sm font-bold text-gray-500">Belum ada dokumen LACT yang digenerate.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
