...
<div class="min-h-screen max-w-md mx-auto bg-[#f7f6f2] pb-24">
    ...
    <div class="px-5 -mt-5">
        ...
        <div x-data="{ showPasswordForm: {{ ((isset($errors) && $errors->updatePassword->any()) || session('status') === 'password-updated') ? 'true' : 'false' }} }"
             class="bg-white border border-gray-200 rounded-2xl mt-4 overflow-hidden">

            <button type="button"
                    @click="showPasswordForm = !showPasswordForm"
                    class="w-full px-5 py-4 flex items-center justify-between text-left">
                <span class="font-bold text-sm">Ganti Password</span>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400 transition-transform" :class="showPasswordForm ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
            </button>

            <div x-show="showPasswordForm" x-collapse class="px-5 pb-5 border-t border-gray-100 pt-4">
