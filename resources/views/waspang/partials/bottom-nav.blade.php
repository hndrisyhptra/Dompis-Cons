    ...
    <div class="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-100 dark:border-slate-800 shadow-[0_-8px_24px_-8px_rgba(30,27,75,0.12)] px-3 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))]">
        ...
        <div class="grid grid-cols-5 items-end text-center text-xs">
            ...
            <a href="{{ route('surveyor.index') }}"
                ...
                <div class="mx-auto w-14 h-14 rounded-2xl bg-[#1565D8] text-white flex items-center justify-center shadow-lg shadow-slate-900/20 ring-4 ring-white dark:ring-slate-900 transition active:scale-95">
                    <i class="fa-solid fa-location-dot text-xl"></i>
                </div>
                <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-1">Survey</p>
            </a>

            <a href="{{ route('waspang.notifications') }}"
                class="flex flex-col items-center gap-1 py-1 rounded-xl transition {{ $active == 'notif' ? 'text-[#1565D8] dark:text-blue-400' : 'text-slate-400 dark:text-slate-500' }}">
                <i class="fa-solid fa-bell text-lg"></i>
                <span class="text-[10px] font-bold">Notif</span>
            </a>

            <a href="{{ route('waspang.profile') }}"
                class="flex flex-col items-center gap-1 py-1 rounded-xl transition {{ $active == 'profil' ? 'text-[#1565D8] dark:text-blue-400' : 'text-slate-400 dark:text-slate-500' }}">
